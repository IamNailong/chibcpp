...
std::unique_ptr<Node> Parser::newBinary(NodeKind Kind, std::unique_ptr<Node> Lhs,
...
std::unique_ptr<Node> Parser::newUnary(NodeKind Kind, std::unique_ptr<Node> Expr) {
  auto N = newNode(Kind);
  N->Lhs = std::move(Expr);
  return N;
}
...
std::unique_ptr<Node> Parser::equality(Token **Rest, Token *Tok) {
  ...
  for (;;) {
    if (Lexer::equal(Tok, "==")) {
      N = newBinary(NodeKind::Eq, std::move(N), relational(&Tok, Tok->Next.get()));
      continue;
    }

    if (Lexer::equal(Tok, "!=")) {
      N = newBinary(NodeKind::Ne, std::move(N), relational(&Tok, Tok->Next.get()));
      continue;
    }
